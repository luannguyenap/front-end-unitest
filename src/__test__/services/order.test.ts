...
describe('OrderService', () => {
  ...
  it('should ensure total price does not go below 0 after applying a coupon', async () => {
    const validOrder = {
      items: [{ id: '1', productId: 'p1', price: 50, quantity: 1 }],
      couponId: '123',
    };
  ...
  it('should throw an error if order creation fails', async () => {
    ...
    (fetch as any)
      .mockResolvedValueOnce({ json: vi.fn().mockResolvedValueOnce(null) }) // Mock coupon API
      .mockResolvedValueOnce({ json: vi.fn().mockResolvedValueOnce(null) }); // Mock failed order API

    paymentService.buildPaymentMethod = vi.fn().mockReturnValue('mock-payment-method');

    await expect(orderService.process(validOrder)).rejects.toThrow('Failed to create order'); // Expecting an error due to failed order creation
  });

  // 6. Payment Handling
  it('should call payViaLink with the created order', async () => {
    const validOrder = {
      items: [{ id: '1', productId: 'p1', price: 100, quantity: 1 }], discount: 90,
    };
